...
if (config.runAsUser) {
    dropProcessPrivs(config.runAsUser);
}

// Reflector booted! Print out welcome message.
const url = config.useTls ? `https://${config.hostname}:${config.https}` : `http://${config.hostname}:${config.http}`;
...
function initFileWatchingCacheInvalidator(cache, watchRecursive='.') {
    ...
    chokidar.watch(watchRecursive, {
        ...
        if (fileName.endsWith('.js')) {
            fileName = fileName.replace('.js', '.md') + ` based on ${fileName}`;
        }
