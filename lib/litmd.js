...
function readTemplates(){
  ...
  return {markdownDefaultImports, headerTemplate, footerTemplate};
}
 ...
 * @returns {string}
 */
function buildHtmlFromLiterateMarkdown(maybeMarkdownString, fileName='', templateData, templates=readTemplates()){
  if (typeof maybeMarkdownString === 'string' || !fileName.endsWith('.md')){
    return maybeMarkdownString;
  }
  templateData.title = generateTitle(fileName);
  ...
  if (hasExplicitHTMLHeader){
    ...
    header = renderTemplate(templates.headerTemplate, templateData);
    body = markdownString;
  }
  return header + litmd.makeHtml(body) + renderTemplate(templates.footerTemplate, templateData);
}
...
function makeMarkdownConverter (options={}) {
  ...
  const result = new showdown.Converter(
    Object.assign({
        ...
        defaultImport: readTemplates().markdownDefaultImports,
        extensions: [
