...
// Calculate path relative to package root for URL generation
const relativePath = path.relative(packageRoot, invocationDir);
const urlPrefix = relativePath ? relativePath + '/' : '';

let debug = false;
if (debug){
    console.log({packageRoot, invocationDir, relativePath, urlPrefix});
...
function getPostMeta(fileName) {
    ...
    // Fallback to filename (without path and extension) if no title
    if (!meta.title) meta.title = path.basename(fileName).replace(/\.(md|html)$/, '');
...
const SKIP_DIRS = ['node_modules', '.git', 'vendor'];

// Recursively get all md/html files from directory and subdirectories
        ...
        if (entry.isDirectory()) {
            if (!SKIP_DIRS.includes(entry.name)) {
                results = results.concat(getFilesRecursive(fullPath, baseDir));
                   ...
                   && entry.name !== 'index.md' && entry.name !== 'index.html') {
...
function getSortedFileNames() {
    return getFilesRecursive(invocationDir);
}
...
const generateIndexFile = (fileNames) => {
    ...
    const content = posts.map((post, index) =>
        `${index + 1}. [${post.title}](/${urlPrefix}${post.fileName})${post.description ? ' - ' + post.description : ''}`)
        .join('\n');
