...
const createLogger = (method) => (...args) => {
  ...
  if (isNode) {
    ...
    // TODO: Nice idea, but is only ever prints the full path of core.js
    // pathname = fileURL.pathname;
  } else {
    pathname = window.location.pathname;
  }
  consoleMethod(`${pathname}`,...args);
};
...
const mapObject = (a,fn) => Object.fromEntries(Object.entries(a).map(fn)) //fn([k,v]) => [k,v2]
...
// structuredClone introduced in node 17+
